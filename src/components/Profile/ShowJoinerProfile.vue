            ...
            <v-col class="py-0">
              <v-list-item color="rgba(0, 0, 0, .4)" dark>
                <v-list-item-content v-if="loginUser.email!=userInfo.email">
                  <v-list-item-title class="title">{{ userInfo.displayName }}</v-list-item-title>
...
export default {
  ...
  methods: {
    addFavoriteUser: function() {
      ...
      userRef.update({
        favoriteUser: firebase.firestore.FieldValue.arrayUnion(this.email)
      });
    ...
    removeFavoriteUser: function() {
      ...
      userRef.update({
        favoriteUser: firebase.firestore.FieldValue.arrayRemove(this.email)
      });
